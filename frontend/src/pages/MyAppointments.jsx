...
const MyAppointments = () => {
  const {backendUrl,data,token, getDoctorsdata} = useContext(AppContext)
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-700 border-b border-stone-400'>My Appointments</p>
      <div>
        {
          appointments.map((item,index)=>(
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b border-stone-400'  key={index}>
              <div>
              ...
              <div className='flex flex-col gap-2 justify-end'>
                {!item.cancelled && item.payment && !item.isCompleted &&  <button className='sm:min w-48 py-2 border rounded text text-stone-500 bg-indigo-50 '>Paid</button>}
